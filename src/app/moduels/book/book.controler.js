...
import *  as authoerModel from "../../DB/models/author.js"

export const addBook = async (req, res) => {
    const { title, content, author } = req.body
    if (title && content && author) {

        let data = await authoerModel.default.findOne({ name: author })
        if (data) {
            let result = await book.create({ title, content, author: { name: author, _id: data._id } })
                .then(() => {
            ...
            let theNewBook = await book.findOne({ title, content, author: { name: author, _id: data._id } })

            let addBook = await authoerModel.default.findOneAndUpdate({ name: author }, { books: [...data.books, theNewBook._id] })
    ...
    //     let data = await authoerModel.default.findOne({ name: result[i].author })
    //     await book.create({title : result[i].title , content: result[i].content,author: {name : result[i].author , _id : data._id}})
    //     let theNewBook = await book.findOne({ title : result[i].title , content: result[i].content,author: {name : result[i].author , _id : data._id}})
    //     let addBook = await authoerModel.default.findOneAndUpdate({ name: result[i].author },{books:[...data.books,theNewBook._id]})
    // }
    // res.status(200).json({
    //     message: "Book added successfully"
    // })
...
export const getBook = async (req, res) => {
    try {
        ...
        let data = await book.findById(req.params.id).select('-author.name').populate({
            path: 'author._id',
...
//     let validBooks = books.filter(book => book.author != "Unknown")

//     let result = validBooks.map((book) =>{
//         return {title: book.title, author: book.author,content:book.title}
//     })

//     export let bookAuthors = result.map(book => book.author)
